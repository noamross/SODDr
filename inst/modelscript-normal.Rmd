...
```{r}
Rprof()
pop.df <- SODModel(treeparms.df,locations,time.steps,init, stochastic.d=TRUE)
Rprof(NULL)
...
```{r}
require(grid)
popp <- subset(pop3.df.totals, Species!="Bay Laurel")
popp$SizeClass[which(popp$Species=="Redwood")] <- 4
dynamic.plot <- ggplot(popp, 
                       aes(x=Time,y=TotPop, fill=Disease, alpha=SizeClass)) + 
                  geom_area(position="stack") + facet_grid(~Species) +
    xlab("Time (years)") + ylab("Fraction of population") +
    scale_alpha_discrete(name="Size Class") +
...
dynamic.plot
ggsave("~/Dropbox/openquals/modelplot.svg",dynamic.plot, height=5, width=9)
```
...
tanspc$Species <- NULL
tanspc <- ddply(tanspc, c("Time","Location"), function(x) {data.frame(TotTan=sum(x$Population[which(x$SizeClass=="3" | x$SizeClass=="4")]), FracDisease=sum(x$Population[which(x$Disease=="I" & (x$SizeClass=="3" | x$SizeClass=="4"))])/sum(x$Population[which(x$SizeClass=="3" | x$SizeClass=="4")]))}, .progress="text")
